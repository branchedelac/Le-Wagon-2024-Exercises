...
def preprocess(min_date: str = "2009-01-01", max_date: str = "2015-01-01") -> None:
    ...
    data = get_data_with_cache(GCP_PROJECT, query, data_query_cache_path, True)
    ...
    data = pd.concat(
        [
            X["pickup_datetime"].reset_index(drop=True),
            pd.DataFrame(X_processed).add_prefix("feat").reset_index(drop=True),
    ...
    load_data_to_bq(
        data, GCP_PROJECT, BQ_DATASET, f"processed_{DATA_SIZE}", truncate=True
    )
...
def train(
    ...
    data_query_cache_path = Path(LOCAL_DATA_PATH).joinpath(
        "processed", f"query_{min_date}_{max_date}_processed_{DATA_SIZE}.csv"
    )

    data_processed = get_data_with_cache(
        GCP_PROJECT, query, data_query_cache_path, True
    )
    data_processed = data_processed.drop("pickup_datetime", axis=1)
    # Create (X_train_processed, y_train, X_val_processed, y_val)

    train_length = int(len(data_processed) * (1 - split_ratio))

    data_train = data_processed.iloc[:train_length, :].sample(frac=1)
    data_val = data_processed.iloc[train_length:, :].sample(frac=1)
