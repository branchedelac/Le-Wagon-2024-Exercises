...
def notify(old_mae, new_mae):
    ...
    channel = '1538' # Change to your batch number
    url = f"{base_url}/{channel}/messages"
    author = 'branchedelac' # Change this to your github nickname
...
def train_flow():
    ...
    # Define the DAG
    preprocessed_new_data = preprocess_new_data.submit(min_date, max_date)

    old_mae = evaluate_production_model.submit(
        min_date, max_date, wait_for=[preprocessed_new_data]
    ).result()

    new_mae = re_train.submit(
        min_date, max_date, split_ratio=0.2, wait_for=[preprocessed_new_data]
    ).result()


    notify.submit(old_mae, new_mae, wait_for=[old_mae, new_mae])

    if new_mae < old_mae:
        transition_model.submit("staging", "production", wait_for=[old_mae, new_mae])
