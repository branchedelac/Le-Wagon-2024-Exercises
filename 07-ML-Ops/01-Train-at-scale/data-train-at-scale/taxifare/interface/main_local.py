...
def preprocess_and_train(
    min_date: str = "2009-01-01", max_date: str = "2015-01-01"
) -> None:
    """
    ...
    data_train = data.iloc[:train_length, :].sample(
        frac=1
    )  # Shuffle datasets to improve training
    data_val = data.iloc[train_length : train_length + val_length, :].sample(frac=1)
    ...
    X_train_preprocessed, X_val_preprocessed = preprocess_features(X_train), preprocess_features(X_val)
    ...
    # Save trained model
    params = dict(learning_rate=learning_rate, batch_size=batch_size, patience=patience)
