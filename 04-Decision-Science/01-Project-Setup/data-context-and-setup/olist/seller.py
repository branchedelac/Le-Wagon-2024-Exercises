...
class Seller:
    ...
    def get_review_score(self):
        ...
        """
        order_items = self.order.data["order_items"][["order_id", "seller_id"]]
        review_scores = self.order.get_review_score()
        items_with_reviews = order_items.merge(review_scores, how="left", on="order_id")

        # Group reviews (already grouped by order) per seller
        sellers_with_reviews = (
        ...
        sellers_with_reviews["dim_is_one_star"] = (
            sellers_with_reviews["dim_is_one_star"] / sellers_with_reviews.shape[0]
        )
        sellers_with_reviews["dim_is_five_star"] = (
            sellers_with_reviews["dim_is_five_star"] / sellers_with_reviews.shape[0]
        )

        return sellers_with_reviews.dropna()

    # Optional
    def get_review_score_sum(self):
        """
        Returns a DataFrame with:
        'seller_id', 'share_of_five_stars', 'share_of_one_stars', 'review_score'
        """
        order_items = self.order.data["order_items"][["order_id", "seller_id"]]
        review_scores = self.order.get_all_review_score_counts()
        items_with_reviews = order_items.merge(review_scores, how="left", on="order_id")
    ...
    def get_training_data(self):
        ...
        reviews_per_order = self.get_review_score()

        if reviews_per_order is not None:
            reviews_per_seller = reviews_per_order.groupby("seller_id").agg(
                {
                    "review_score": "mean",
                    "review_cost": "sum",
                    "dim_is_one_star": "sum",
            ...
            training_set = training_set.merge(reviews_per_seller, on="seller_id")
